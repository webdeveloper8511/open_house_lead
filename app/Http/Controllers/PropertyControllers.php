...
class PropertyControllers extends Controller
{
    ...
    {
        $perportys = Perporty::get();
        $riderCodes = RiderCode::get();
        return view('dashboard.property.property',compact('perportys','riderCodes'));
    }

    public function propertyCreate()
    {
        $headlineTextColor = HeadlineTextColor::get();
        $headlineColor = HeadlineColor::get();
        return view('dashboard.property.add-edit-property-dt',compact('headlineTextColor','headlineColor'));
    }
    ...
    public function propertyAdd(Request $request)
    {
        ...
        $propertyOptions->headline_text_color = $request->headline_text_color;
    ...
    public function propertyEdit(Request $request,$id)
    {
        $headlineTextColor = HeadlineTextColor::get();
        $headlineColor = HeadlineColor::get();
        $property = Perporty::where('id',$id)->with('propertyImage','propertyOption','PropertySmartphone')->first();
        return view('dashboard.property.add-edit-property-dt-edit',compact('property','headlineTextColor','headlineColor'));
    }

    public function propertyUpdate(Request $request,$id)
    {
        return $request->all();
        $property = Perporty::where('id',$id)->first();
        $property->street = $request->street;
        $property->city = $request->city;
        $property->state = $request->state;
        $property->zip_code = $request->zip_code;
    ...
    public function riderCodes(Request $request)
    {
        $ridercode = Perporty::where('id',$request->property_id)->first();
        $ridercode->rider_code = $request->rider_code;
        $ridercode->update();
        return redirect("property")->with('success','Rider Code Add Successfully');
    }
}
